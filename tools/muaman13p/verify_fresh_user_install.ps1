# verify_fresh_user_install.ps1 - MUAMAN-13P orchestrator.
...
# (CreateProcessWithLogonW), collects the worker evidence, computes the P1..P14
...
[CmdletBinding()]
param(
    [string]$WorktreePath = 'C:\dev\muaman.worktrees\muaman-13p-independent-fresh-user-install-first-launch-acceptance',
    [string]$MainRepoPath = 'C:\dev\muaman',
    [string]$AcceptanceRoot = 'C:\mu13o-acceptance\m13p',
    [string]$InstallerSource = 'C:\mu13o-acceptance\run1\consumer\inbound\muaman-windows-installer.exe',
    [string]$CredFile = 'C:\Users\saber\AppData\Local\Temp\opencode\m13p-cred.txt',
    [string]$AccountName = 'CodexMuaman13P',
...
$m13pEvidenceDest = Join-Path $WorktreePath 'docs\muaman-13p\evidence'
$runEvidenceDest = Join-Path $m13pEvidenceDest $RunId
...
# the wrapper reads, exports as M13P_* env vars, then deletes. The child itself
...
# ------------------------------------------------------------ 6. guard gates
Write-Output '[6/9] compute P1..P14 gates'
& (Join-Path $toolsDir 'guard_tests_13p.ps1') `
...
Copy-Item -Path (Join-Path $runRoot '*') -Destination $runEvidenceDest -Recurse -Force
Copy-Item -LiteralPath (Join-Path $runRoot 'guards-result.json') -Destination (Join-Path $runEvidenceDest 'guards-result.json') -Force -ErrorAction SilentlyContinue
...
# Guards verdict is authoritative; guard_tests_13p.ps1 exits nonzero on failure.
