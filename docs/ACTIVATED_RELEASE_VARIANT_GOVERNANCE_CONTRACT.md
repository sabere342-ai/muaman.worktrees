...
- **Owner authorization master switch:** `ownerAuthorizationActive` governs whether any
  combination of inputs MAY reach `ACTIVATED`. When `ownerAuthorizationActive=false`, NO
  combination of other inputs can produce ACTIVATED. When `true`, it is necessary but NOT
  sufficient — activation still requires the complete positive authorization surface, so active
  owner authorization alone does not activate.
...
```
ACTIVATED_VARIANT_1_APPROVAL_DIGEST = 64E3123C9B809B1C6B63EB737003AE61FD4557693888BD74C3BD7EEDC5310D59
ACTIVATED_VARIANT_1_ALLOWED_ENVS    = ["production"]
OWNER_AUTHORIZATION_ACTIVE           = true
ACTIVATED_VARIANT_1_APPROVED_SOURCE_COMMIT = 56526f39565c64531b4f1dfef22d060506d56479
```

**OWNER AUTHORIZATION RECORDED:** The human owner explicitly authorized
`OWNER_AUTHORIZATION_PREPARATION_FOR_ACTIVATED_VARIANT_1_ONLY` for source baseline
`56526f39565c64531b4f1dfef22d060506d56479`. The committed digest above is the **approval
identity digest** recomputed internally from the owner approval artifact. This records the
owner authorization governance state ONLY.

**COMMIT BINDING — APPROVED vs GOVERNANCE COMMIT:** `56526f39565c64531b4f1dfef22d060506d56479`
is the **APPROVED_SOURCE_COMMIT** (the exact locked source baseline authorized to be built by a
future activation session). The commit that records this authorization is the
**AUTHORIZATION_GOVERNANCE_COMMIT** and is a separate, distinct SHA. The approval digest is
bound to the APPROVED_SOURCE_COMMIT and does NOT change when the authorization governance
commit is recorded on top of it. A future activated build claims `56526f...` as
`sourceCommit` — never the governance commit. There is NO circular/self-referential dependency.

`ownerAuthorizationActive=true` and the presence of the committed digest do NOT, by themselves,
authorize or execute activation. Activation additionally requires the complete positive
authorization surface: the file-bound approval artifact matching this digest + production
environment + explicit opt-in + capability seam, and the normal default build stays
`NORMAL_GATED_OFF`.
...
the authorized marker. Even though `ownerAuthorizationActive=true`, a matching digest alone
does NOT authorize activation — the complete positive authorization surface is still required,
and a normal/default build remains `NORMAL_GATED_OFF`.
...
- the committed contract is not active (`ownerAuthorizationActive=false`) — i.e., the
  verifier only ACCEPTS an otherwise fully-valid bundle when the owner authorization contract
  is active and every other provenance/fingerprint/commit check passes.
...
- **PRODUCTION CONTRACT TESTS** (G1-G20, G25-G28, G30-G32): prove that, even with
  `ownerAuthorizationActive=true` and the authorized approval digest recorded, activation
  requires the COMPLETE positive authorization surface. Default build, capability-only, and
  active-owner-authorization-alone all stay OFF; every partial/malformed/mismatched/expired/
  stale case stays OFF or REFUSED.
- **POSITIVE-PATH PROOF** (G21): exactly one fully valid positive-path case resolves
  `ACTIVATED` in the inert classifier against the real owner-authorized contract state. This is
  classification-only proof; it does not trigger build, drain, deployment, shipment, or
  production contact. (G29: verifier accepts a fully valid real evidence bundle; classification only.)
