...
IDENTITY       = ACTIVATED_RELEASE_VARIANT_GOVERNANCE_AND_TOOLING (governance + inert tooling only)
DRAIN_STATE    = GATED/OFF
AUTHORITY      = governance definition + inert fail-closed tooling ONLY
NO_ACTIVATION  = this document defines a future contract; it does not authorize or perform activation
```
...
| Owner-approval file present but wrong/absent variant ID | `NOT_AUTHORIZED` | OFF |
| Malformed variant ID or malformed approval digest | `NOT_AUTHORIZED` | OFF |
| Unauthorized / debug / local environment | `NOT_AUTHORIZED` | OFF |
...
1. **Variant identity (compile-time):** the exact authorized `-VariantId` matching
   `ACTIVATED_VARIANT_1`.
2. **Owner approval artifact (file, not committed):** a file whose SHA-256 matches the
   digest recorded for this variant in the committed contract (see §8). The file itself is
   NOT committed.
...
| Tool | Purpose |
|---|---|
| `resolve_release_variant.ps1` | Classifies a release variant from explicit inputs per the fail-closed matrix. Emits JSON evidence. Never activates. |
| `verify_activated_release.ps1` | Given a produced artifact evidence bundle, verifies variant identity, commit provenance, and authorization digest. Refuses anything not fully authorized. Never activates. |
| `guard_tests_activated_variant.ps1` | Deterministic guard harness that proves the fail-closed matrix using inert test doubles. Never contacts production. |
...
`ACTIVATED_VARIANT_1`. The digest value is the SHA-256 of the literal string that a future
owner-authorization must reproduce. **The digest alone does not authorize activation** — it
is the comparison target for the separate approval artifact.

```
ACTIVATED_VARIANT_1_APPROVAL_DIGEST = <see resolver; committed reference digest>
...
- `approvalDigestSha256` = digest that matched the owner-approval marker
- `releaseVariantFingerprint` = SHA-256 of the canonical variant identity (source commit +
  variant id + build time) — used to prove exactly which artifact was activated.
...
for every case in the fail-closed matrix except the fully-authorized case, and the fully
authorized case itself requires a separately authorized session to supply the non-committed
approval artifact.

## 11. What a Later Owner-Authorized Activation Session Must Answer
...
7. How artifact provenance is proven → `verify_activated_release.ps1` fingerprint chain.
8. What owner approval is needed → the non-committed approval artifact matching the digest.
9. What production preflight must pass → a future production-preflight gate (see §12).
10. Exact stop conditions → the fail-closed matrix `NOT_AUTHORIZED` outcomes.
...
13. What proves shipped binary corresponds to approved commit → `releaseVariantFingerprint`.
...
## 12. Future Production Preflight (referenced, not performed)
...
- This session defined the contract and the inert fail-closed tooling.
...
- `tools/release/resolve_release_variant.ps1` — fail-closed variant classifier.
- `tools/release/verify_activated_release.ps1` — provenance / stale-artifact guard.
- `tools/release/guard_tests_activated_variant.ps1` — deterministic fail-closed proof.
